...
class DepRelativeMultiheadAttention(MultiheadAttention):
    ...
    def get_relative_embedding(self, keys):
        if keys.dtype == torch.long:
            return self.relative_positions_embeddings(keys)
        else:
            _, _, _, class_num = keys.shape
            return keys @ self.relative_positions_embeddings.weight[:class_num]
    ...
    def forward(
        ...
        query_key = torch.matmul(q, k.transpose(1, 2))
        ...
        relations_keys = self.get_relative_embedding(relative_positions_matrix.to(key.device))

        relations_values = self.get_relative_embedding(relative_positions_matrix.to(key.device))
