        ...
        for obj in outputs:
            if outputs[obj].get("loss", None) is None:
                _losses = self._compute_loss(
                    ...
                    ctc_mask=outputs[obj].get("ctc_mask", None),
                    obj=obj,
                )
            else:
...
class nat_ctc_loss(GenericLMCriterion):
    def __init__(self, task, label_smoothing):
        super(nat_ctc_loss, self).__init__(task, label_smoothing)
        self.unk = task.target_dictionary.unk()

    def _compute_loss(
            self, outputs, targets, masks=None, label_smoothing=0.0, name="loss", factor=1.0, obj="", **kwargs
    ):
        if obj != "word_ins":
            return super()._compute_loss(outputs, targets, masks=None, label_smoothing=0.0, name="loss", factor=1.0,
                                         **kwargs)
        ...
        if masks is not None and not masks.any():
            ...
            logits = F.log_softmax(outputs, dim=-1).transpose(0, 1)  # [seq_len, batch_size, vocab_size]
            logits_mask = kwargs['ctc_mask']
            logits_length = (~logits_mask).long().sum(-1)
            target_length = masks.long().sum(-1)
            losses = F.ctc_loss(
                logits.float(),
                targets,
                logits_length,
            ...
            if label_smoothing > 0:
                smoothed_loss = -logits.mean(-1)[logits_mask.bool()].mean()
