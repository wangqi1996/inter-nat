...
class NAT(NATransformerModel):
    ...
    def forward_decoder(
        ...
        # execute the decoder
        output_masks = output_tokens.eq(self.unk)  # bos eos and unk
        # output_masks = output_tokens.ne(self.pad)
        _scores, _tokens = self.decoder.forward(
            normalize=False,
...
class NATDecoder(NATransformerDecoder):
    ...
    def extract_features(
        ...
        _encoder_padding_mask = encoder_out.encoder_padding_mask if encoder_out is not None else None
...
@register_model_architecture('iwslt16', 'nat_toy')
def nat_toy(args):
...
@register_model_architecture('iwslt16', 'nat_iwslt14_en_de')
